    ...
    public MainFrame() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                switch (result) {
                    case JFileChooser.APPROVE_OPTION:
                        ...
                        Graph map = MapFile.readMap(mapChooser.getSelectedFile().getAbsolutePath());
                        System.out.println(" --- Init map Panel");
                        mapPanel.setMap(map);

                        add(mapPanel, BorderLayout.CENTER);
                        System.out.println(" --- Done!");
                        map.addVertex(new Point("PAris",48.8567, 2.3508));
                        revalidate();
                         repaint();
                        break;
                    default:
        ...
        addPlace.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                /**
                 * String[] buttons = {"Add Place", "Cancel"}; int option =
                 * JOptionPane.showOptionDialog(null, inputPanel, "Add place to
                 * map:", JOptionPane.WARNING_MESSAGE, 0, null, buttons,
                 * buttons[1]); if (option == 0) {
                 *
                 * //create place Vertex place = new Vertex(txtPlace.getText(),
                 * Float.parseFloat(txtLatitude.getText()),
                 * Float.parseFloat(txtLongitude.getText()));
                 *
                 * if (cmbNeighbours.getSelectedItem() != null) { //connect
                 * place to it destination Vertex dest = map.getVertex((String)
                 * cmbNeighbours.getSelectedItem());//find destination using
                 * name map.addBiEdge(place, dest); } else {
                 * map.addVertex(place); } JOptionPane.showMessageDialog(null,
